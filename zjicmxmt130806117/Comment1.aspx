<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Comment1.aspx.cs" Inherits="Comment" %>
...
<body>
    ...
    <div align="center">
        <br />
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="Data Source=PC731\SQLEXPRESS;Initial Catalog=Date;Persist Security Info=True;User ID=sa;Password=good6565" ProviderName="System.Data.SqlClient" SelectCommand="SELECT [comment], [topic_name], [user_name], [comment_id] FROM [Table_comment] WHERE ([topic_name] = @topic_name)" ConflictDetection="CompareAllValues" DeleteCommand="DELETE FROM [Table_comment] WHERE [comment_id] = @original_comment_id AND (([comment] = @original_comment) OR ([comment] IS NULL AND @original_comment IS NULL)) AND (([topic_name] = @original_topic_name) OR ([topic_name] IS NULL AND @original_topic_name IS NULL)) AND (([user_name] = @original_user_name) OR ([user_name] IS NULL AND @original_user_name IS NULL))" InsertCommand="INSERT INTO [Table_comment] ([comment], [topic_name], [user_name]) VALUES (@comment, @topic_name, @user_name)" OldValuesParameterFormatString="original_{0}" UpdateCommand="UPDATE [Table_comment] SET [comment] = @comment, [topic_name] = @topic_name, [user_name] = @user_name WHERE [comment_id] = @original_comment_id AND (([comment] = @original_comment) OR ([comment] IS NULL AND @original_comment IS NULL)) AND (([topic_name] = @original_topic_name) OR ([topic_name] IS NULL AND @original_topic_name IS NULL)) AND (([user_name] = @original_user_name) OR ([user_name] IS NULL AND @original_user_name IS NULL))">
            <DeleteParameters>
            ...
            <SelectParameters>
                <asp:Parameter DefaultValue="盗墓笔记" Name="topic_name" Type="String" />
            </SelectParameters>
            <UpdateParameters>
                <asp:Parameter Name="comment" Type="String" />
                <asp:Parameter Name="topic_name" Type="String" />
